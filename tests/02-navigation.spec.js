...
// Functional navigation checks (link integrity, HTTP 200, back/forward, title).
// Uses plain Playwright on purpose — Playwright screenshots are still saved to
// disk via BasePage.takeScreenshot(); only the Chromatic visual snapshots are skipped.
...
test.describe('Navigation', () => {
  ...
  test('Browser back/forward buttons work', async ({ page }) => {
    ...
    expect(forwardURL).toContain(host);
  ...
  test('All internal links return HTTP 200 (no broken links)', async ({ page }) => {
    ...
    for (const link of links) {
      ...
      const host = new URL(BASE_URL).hostname;
      if (!fullURL.includes(host)) continue;
