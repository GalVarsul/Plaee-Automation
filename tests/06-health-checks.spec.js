...
// Purely functional checks (HTTP status, HTTPS, performance, console/network).
// Uses plain Playwright on purpose — these tests have no visual output, so they
// should NOT produce Chromatic snapshots.
...
test.describe('Health - HTTPS & Security', () => {
  ...
  test('All internal category hrefs use HTTPS (no mixed content)', async ({ page }) => {
    ...
    for (const link of links) {
      ...
      // Relative paths inherit the page protocol and are safe.
      // Only explicit http:// links are mixed content.
      if (href && href.startsWith('http://')) {
        mixedContent++;
        console.log(`  Mixed content link: ${href}`);
      }
    }

    console.log(`${links.length} category links checked — ${mixedContent} mixed content`);
    expect(mixedContent, 'No links should use http:// explicitly').toBe(0);
  });
