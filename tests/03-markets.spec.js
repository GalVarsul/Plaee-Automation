...
const SPORTS_URL = `${BASE_URL}/category/sports`;

// Reuse NAV_CATEGORIES from constants — skip 'All' (it's just the homepage)
const MARKET_CATEGORIES = NAV_CATEGORIES.slice(1);
...
test.describe('Markets - Detail Page', () => {
  ...
  test('Market detail page has all required elements', async ({ page }) => {
    ...
    // The page should always be a valid site page
    expect(url).toContain(new URL(BASE_URL).hostname);
  ...
  test('Can navigate back from market detail to listing', async ({ page }) => {
    ...
    // Should be back on the site (sports or homepage)
    expect(urlAfter).toContain(new URL(BASE_URL).hostname);
